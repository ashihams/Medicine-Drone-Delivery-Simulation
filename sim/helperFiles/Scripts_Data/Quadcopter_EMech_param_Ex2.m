...
% Size of the ground
planex = 25;                    % m  it was 12.5
planey = 25;                    % m  it was 12.5
...
pkgSize = [1 1 1]*0.15; % m
pkgDensity = 0.1/(0.15^3); % kg/m^3   -- 100g cargo

%% Motor Parameters
p    = 2;        % Number of pole pairs
Rs   = 0.1/10;   % Stator resistance per phase           [Ohm]
Ls   = 1e-4;     % Stator self-inductance per phase, Ls  [H]
Ms   = 1e-5;     % Stator mutual inductance, Ms          [H]
psim = 0.0175/4; % Maximum permanent magnet flux linkage [Wb]
Jm   = 0.0005;   % Rotor inertia                         [Kg*m^2]
Vdc  = 14.8;     % DC link voltage                       [V]

Imax = 35;
Tmax = 2*p*psim*Imax;
7000*2*pi/60*p*psim*2;

motorDamping = 4e-4; %N*m/(rad/s)

%% Motor speed control parameters
%Ts  = 5e-5;  % Fundamental sample time            [s]
%Tsc = 5e-5;  % Sample time for inner control loop [s]
%Kpw = 0.1;   % Proportional gain speed controller
%Kiw = 1.4;    % Integrator gain speed controller
